...
public:
	 ...
	 */
	//Device() : base_type(system::default_device()) {
	//}

	/** Takes a shared_ptr to an implementation
	 */
	Device(shared_ptr_to_impl base_ptr) : base_type(base_ptr) {
	}
...
private:
	...
	// base_type = detail::shared_ptr_impl<Device, detail::device>
	using base_type = typename Device::shared_ptr_impl;

	// shared_ptr_to_impl = std::shared_ptr<detail::device>
	using shared_ptr_to_impl = typename base_type::shared_ptr_to_impl;
